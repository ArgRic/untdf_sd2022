...
div_prog_1(struct svc_req *rqstp, register SVCXPRT *transp)
{
	...
	case DIV_RPC:
		_xdr_argument = (xdrproc_t) xdr_div_args;
		_xdr_result = (xdrproc_t) xdr_long;
		local = (char *(*)(char *, struct svc_req *)) div_rpc_1_svc;
		break;
...
main()
{
	...
	if (!ioctl(0, I_LOOK, mname) &&
		...
		if (!svc_reg(transp, DIV_PROG, TP4, div_prog_1, 0)) {
			_msgout ("unable to register (DIV_PROG, TP4).");
			exit(1);
		}
		if (pmclose) {
			(void) signal(SIGALRM, (SIG_PF) closedown);
		...
		(void) dup2(i, 2);
		setsid();
		openlog("division", LOG_PID, LOG_DAEMON);
#endif
	}
	if (!svc_create (div_prog_1, DIV_PROG, TP4, "netpath")) {
 		_msgout ("unable to create (DIV_PROG, TP4) for netpath.");
		exit (1);
	}
